...
function filterAsteroids(asteroids) {
  ...
  for (var i = 0; i < combinedAsteroids.length; i++) {
    var closeApproachData = combinedAsteroids[i].close_approach_data[0];
    var closeApproachDateTime = new Date(closeApproachData.close_approach_date_full);
  ...
  return new Promise((resolve, reject) => {
    fetch(url)
      ...
      .then(data => {
        ...
        var filteredAsteroids = filterAsteroids(asteroids);
        var jsonData = JSON.stringify(filteredAsteroids);
        // saveToFile(jsonData, 'objects.json');
        socket.emit('neo', jsonData);
        resolve(jsonData);
      })
      .catch(error => {
